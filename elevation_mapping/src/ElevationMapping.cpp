...
void ElevationMapping::pointCloudCallback(sensor_msgs::msg::PointCloud2::ConstSharedPtr pointCloudMsg, bool publishPointCloud,
  ...
  // Process point cloud.
  // RCLCPP_INFO_STREAM(nodeHandle_->get_logger()," robot pose Variance is:"<<robotPoseCovariance);
  ...
  // RCLCPP_INFO_STREAM(nodeHandle_->get_logger(),"measurement Variance is:"<<measurementVariances[0]<<" , "<<measurementVariances[10]<<" , "<<measurementVariances[100]<<" , "<<measurementVariances[110]<<" , "<<measurementVariances[120]);
...
void ElevationMapping::mapUpdateTimerCallback() {
  RCLCPP_INFO(nodeHandle_->get_logger(), "Map update timer triggered at %f", nodeHandle_->get_clock()->now().seconds());
  if (!updatesEnabled_) {
    rclcpp::Clock clock;
    RCLCPP_WARN_THROTTLE(nodeHandle_->get_logger(), clock, 10, "Updating of elevation map is disabled. (Warning message is throttled, 10s.)");
    map_.setTimestamp(nodeHandle_->get_clock()->now());
