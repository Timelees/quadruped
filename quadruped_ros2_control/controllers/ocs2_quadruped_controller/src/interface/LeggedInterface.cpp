...
namespace ocs2::legged_robot
{
                                                     ...
                                                     const bool verbose)
    {
        ...
        // Self-collision avoidance constraint 自碰撞约束
    ...
    void LeggedInterface::setupModel(const std::string& task_file, const std::string& urdf_file,
                                     const std::string& reference_file)
    {
        // PinocchioInterface 创建质心动力学模型pinocchio接口
    ...
    void LeggedInterface::setupPreComputation(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
        ...
        problem_ptr_->preComputationPtr = std::make_unique<LeggedRobotPreComputation>(
            *pinocchio_interface_ptr_, centroidal_model_info_, *reference_manager_ptr_->getSwingTrajectoryPlanner(), model_settings_);
    }
        ...
        const size_t contact_point_index)
    {
        auto eeZeroVelConConfig = [](scalar_t positionErrorGain)
        {
            ...
            if (!numerics::almost_eq(positionErrorGain, 0.0))
            {
                ...
                config.Ax(2, 2) = positionErrorGain;     // 防止足端漂移
            }
            return config;
        };
