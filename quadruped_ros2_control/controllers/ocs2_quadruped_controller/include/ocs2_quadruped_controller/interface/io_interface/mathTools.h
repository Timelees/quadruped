...
#include <stdio.h>
#include <iostream>
#include "ocs2_quadruped_controller/interface/io_interface/mathTypes.h"
...
inline RotMatd rotx(const double &theta) {
...
inline RotMatd roty(const double &theta) {
    double s = std::sin(theta);
    double c = std::cos(theta);
...
inline RotMatd rotz(const double &theta) {
    double s = std::sin(theta);
    double c = std::cos(theta);
...
inline Mat2d skew(const double& w){
    Mat2d mat; mat.setZero();
    mat(0, 1) = -w;
    mat(1, 0) =  w;
    return mat;
}
...
inline RotMatd rpyToRotMat(const double& row, const double& pitch, const double& yaw) {
    RotMatd m = rotz(yaw) * roty(pitch) * rotx(row);
    return m;
}

inline Vec3d rotMatToRPY(const Mat3d& R) {
    Vec3d rpy;
...
inline RotMatd quatToRotMat(const Quatd& q) {
    double e0 = q(0);
    double e1 = q(1);
    double e2 = q(2);
    double e3 = q(3);
...
inline Vec3d rotMatToExp(const RotMatd& rm){
    ...
    if (fabs(angle) < 1e-5){
        ...
        exp = angle * Vec3d(rm(0,0)+1, rm(0,1), rm(0,2)) / sqrt(2*(1+rm(0, 0)));
    }
    else{
        exp=angle/(2.0f*sin(angle))*Vec3d(rm(2,1)-rm(1,2),rm(0,2)-rm(2,0),rm(1,0)-rm(0,1));
    }
    return exp;
}

inline HomoMatd homoMatrix(Vec3d p, RotMatd m){
    HomoMatd homoM;
    homoM.setZero();
...
inline HomoMatd homoMatrix(Vec3d p, Quatd q){
    HomoMatd homoM;
    homoM.setZero();
...
inline HomoMatd homoMatrixInverse(HomoMatd homoM){
    HomoMatd homoInv;
    homoInv.setZero();
...
inline Vec3d noHomoVec(Vec4d v4){
    Vec3d v3;
    v3 = v4.block(0, 0, 3, 1);
    return v3;
}
...
public:
    AvgCov(unsigned int size, std::string name, bool avgOnly=false, unsigned int showPeriod=1000, unsigned int waitCount=5000, double zoomFactor=10000)
            :_avgOnly(avgOnly), _size(size), _showPeriod(showPeriod), _waitCount(waitCount), _zoomFactor(zoomFactor), _valueName(name) {
        _exp.resize(size);
